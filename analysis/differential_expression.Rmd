...
```{r}
#within the plate, within an organoid, or within a treatment 

#desifgn matrix = Org number, Treated vs untreated, BC vs no BC
```
### Heatmaps
<!-- #### Organoids  -->
...
```{r}
#ORG38
resistance_genes <- c('MFAP2','CLDN1','TSPAN9','MLLT11','CCL26')
counts <-  mac@assays$RNA$counts[resistance_genes,]
barcodes <- metadata[metadata$Organoid=='ORG38' &
                                metadata$Time=='48H',] 
barcodes <- c(barcodes[barcodes$Treatment_1== 'DMSO' |barcodes$Treatment_1== 'Paclitaxel_IC50'|barcodes$Treatment_1== 'SN38_IC50'  , c("Barcode")])
...
resistance_genes <- c('MFAP2','CLDN1','TSPAN9','MLLT11','CCL26')
counts <-  mac@assays$RNA$counts[resistance_genes,]
barcodes <- metadata[metadata$Organoid=='ORG49' &
                                metadata$Time=='48H',] 
barcodes <- c(barcodes[barcodes$Treatment_1== 'DMSO' |barcodes$Treatment_1== 'Paclitaxel_IC50'|barcodes$Treatment_1== 'SN38_IC50'  , c("Barcode")])
counts <- counts[,barcodes]
counts <- as.data.frame(counts)
...
resistance_genes <- c('MFAP2','CLDN1','TSPAN9','MLLT11','CCL26')
counts <-  mac@assays$RNA$counts[resistance_genes,]
barcodes <- metadata[metadata$Organoid=='ORG66' &
                                metadata$Time=='48H',] 
barcodes <- c(barcodes[barcodes$Treatment_1== 'DMSO' |barcodes$Treatment_1== 'Paclitaxel_IC50'|barcodes$Treatment_1== 'SN38_IC50'  , c("Barcode")])
counts <- counts[,barcodes]
counts <- as.data.frame(counts)
