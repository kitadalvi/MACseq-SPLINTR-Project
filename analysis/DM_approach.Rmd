...
Looking at interaction together:
```{r, echo=TRUE}
### Creating new design matrix for interaction between barcode and drug
### factors to include = interaction treatment and concentration
group <- sampleinfo$interaction_condition
...
columns <- metadata[metadata$Organoid=='ORG66' & metadata$Drug=="DMSO" | metadata$Drug=="SN38",]
columns <- c(columns[,c("label")])

data<- as.data.frame(mac@assays$RNA$counts)
genes <- data[de_genes,columns]

type_annotation <- data.frame(Drug=metadata[columns,]$Drug,
                              SPLINTR = metadata[columns,]$SPLINTR,
                              Time = metadata[columns,]$Time_2,
                              Concentration = metadata[columns,]$Concentration)
rownames(type_annotation)<- metadata[columns,]$label
pheatmap(genes, border_color = 'NA', cellwidth = 8, annotation_col = type_annotation, cellheight = 10)
```
...
data<- as.data.frame(mac@assays$RNA$counts)
genes <- v@.Data[[1]][de_genes,]

type_annotation <- data.frame(Drug=metadata$Drug,
                              SPLINTR = metadata$SPLINTR,
                              Time = metadata$Time_2,
                              Concentration = metadata$Concentration)
rownames(type_annotation)<- metadata$label
pheatmap(genes, border_color = 'NA', cellwidth = 8, annotation_col = type_annotation, cellheight = 10)
```
